...
// ─── FDA 21 CFR 101.36 — Nutrient database ───────────────────────────────────
interface NutrientDef {
  label: string;
  unit: 'mg' | 'mcg';
  dv: number | null; // null = no DV established (†)
  order: number;
}
...
const NUTRIENTS: Record<string, NutrientDef> = {
  ...
  // Botanicals / actives — no established DV
...
// ─── FDA rounding rules — 21 CFR 101.9 & 101.36 ─────────────────────────────
// Vitamins & minerals in mg/mcg: round to nearest 0.1 if <10, nearest 1 if ≥10
...
// %DV rounding: <2% → "<2%", 2–10% → nearest 1%, 10–50% → nearest 2%, >50% → nearest 5%
function fmtDV(pct: number): string {
  if (pct < 2)  return '<2%';
  if (pct < 10) return `${Math.round(pct)}%`;
  if (pct <= 50) return `${Math.round(pct / 2) * 2}%`;
...
function Panel({
  ...
  const rows = sorted.map(ing => {
    const n = NUTRIENTS[ing.key];
    if (!n || ing.amount <= 0) return null;
    let dvStr: string;
    if (n.dv === null) { hasNoDV = true; dvStr = '†'; }
    else { hasDV = true; dvStr = fmtDV((ing.amount / n.dv) * 100) + '*'; }
    return { id: ing.id, label: n.label, amtStr: fmtAmount(ing.amount, n.unit), dvStr };
  }).filter(Boolean);
  ...
  return (
    <div style={{
      ...
      {/* Serving size — FDA REQUIRED */}
      <div style={{ fontSize: 10, lineHeight: 1.6 }}>
        {servingSize ? <>Serving Size {servingSize}</> : <span style={{ color: '#c00' }}>⚠ Serving Size required</span>}
      </div>
      {servingsPerContainer && (
      ...
      {/* Nutrient rows */}
      {rows.length === 0 && (
        <div style={{ color: '#aaa', fontSize: 10, padding: '12px 0', textAlign: 'center' }}>
          Add ingredients to generate panel
        </div>
      )}
      {rows.map(row => row && (
        <div key={row.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', padding: '1.5px 0', borderTop: '0.5px solid #000' }}>
          <span>{row.label}</span>
          <span style={{ whiteSpace: 'nowrap', marginLeft: 8 }}>{row.amtStr}&nbsp;&nbsp;{row.dvStr}</span>
        </div>
      ))}
...
export function SupplementFactsPage() {
  ...
  const [servingSize, setServingSize] = useState('2 Gummies (7 g)');
  const [servingsPerContainer, setServingsPerContainer] = useState('30');
  ...
  return (
    <div style={{ minHeight: '100%', padding: 24, background: 'hsl(38,20%,96%)' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <h1 style={{ fontSize: 28, fontWeight: 700, color: 'hsl(0,0%,10%)', margin: 0 }}>
          Supplement Facts Generator
        </h1>
        <p style={{ marginTop: 4, fontSize: 14, color: 'hsl(0,0%,50%)' }}>
          Enter your ingredients and amounts per serving. The FDA-compliant panel generates automatically.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Serving info — FDA required */}
          <div style={{ background: '#fff', border: '0.5px solid hsl(0,0%,88%)', borderRadius: 8, padding: '14px 16px' }}>
            <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em', color: P, marginBottom: 10 }}>
              Required by FDA
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <div>
                <label style={{ fontSize: 11, color: 'hsl(0,0%,50%)', display: 'block', marginBottom: 4 }}>
                  Serving Size <span style={{ color: '#c00' }}>*</span>
                </label>
                <input
                  value={servingSize}
                  onChange={e => setServingSize(e.target.value)}
                  placeholder="e.g. 2 Gummies (7 g)"
                  style={fieldStyle}
                />
              </div>
              <div>
                <label style={{ fontSize: 11, color: 'hsl(0,0%,50%)', display: 'block', marginBottom: 4 }}>
                  Servings Per Container <span style={{ color: '#c00' }}>*</span>
                </label>
                <input
          ...
          <div style={{ background: '#fff', border: '0.5px solid hsl(0,0%,88%)', borderRadius: 8, padding: '14px 16px' }}>
            ...
            {/* Column headers */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 110px 28px', gap: 8, paddingBottom: 6, borderBottom: '0.5px solid hsl(0,0%,92%)', marginBottom: 8 }}>
              <span style={{ fontSize: 11, color: 'hsl(0,0%,55%)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Ingredient</span>
              <span style={{ fontSize: 11, color: 'hsl(0,0%,55%)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Amount / serving</span>
              <span />
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {ingredients.map(ing => {
                const n = NUTRIENTS[ing.key];
                return (
                  <div key={ing.id} style={{ display: 'grid', gridTemplateColumns: '1fr 110px 28px', gap: 8, alignItems: 'center' }}>
                    <select
                      value={ing.key}
                      onChange={e => update(ing.id, 'key', e.target.value)}
                      style={{ ...fieldStyle, padding: '7px 6px', fontSize: 12 }}
                    >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                      <input
                        ...
                        style={{ ...fieldStyle, padding: '7px 6px', fontSize: 12 }}
                      />
                      <span style={{ fontSize: 11, color: 'hsl(0,0%,50%)', whiteSpace: 'nowrap', minWidth: 24 }}>
                        {n?.unit}
                      </span>
                    </div>
                    <button
                      onClick={() => remove(ing.id)}
                      style={{ background: 'none', border: 'none', fontSize: 18, color: 'hsl(0,0%,65%)', cursor: 'pointer', padding: 0, lineHeight: 1 }}
                    >
                      ×
                    </button>
                  </div>
              ...
              {ingredients.length === 0 && (
                <div style={{ textAlign: 'center', padding: '28px 0', fontSize: 14, color: 'hsl(0,0%,65%)' }}>
          ...
          {/* Compliance note */}
          <div style={{ padding: '10px 14px', background: 'hsl(337,62%,96%)', borderRadius: 6, border: '0.5px solid hsl(337,62%,82%)' }}>
            <p style={{ fontSize: 11, color: P, margin: 0, lineHeight: 1.7 }}>
              <strong>FDA compliance applied:</strong> Rounding per 21 CFR 101.9 — amounts to nearest 0.1 below 10, nearest 1 above 10. %DV per 21 CFR 101.36 — &lt;2% shown as "&lt;2%", 2–10% nearest 1%, 10–50% nearest 2%, &gt;50% nearest 5%. Nutrients ordered per 21 CFR 101.36(b)(2). Botanicals receive †. Serving Size and Servings Per Container are required fields under 21 CFR 101.36(b)(1).
            </p>
          </div>
        </div>
